...
// 有限状态机FSM进行HTML解析,html标准中规定了80多种状态.
// 状态机,状态迁移
function data(c) {
  // 开始标签,结束标签,自闭和标签.
  // tag特点 < 作为开始.
  if (c === '<') {
    return tagOpen; //tagOpen 状态,三种情况
  } else if (c === EOF) {
    return;
  } else {
    return data;
...
function parseHTML(html) {
  let state = data; // 初始状态
  for (const c of html) { // 对html字符串的每个字节进行循环解析.
    state = state(c);
  }

  state = state(EOF); // 状态机的最后一个输入: 文件终结符号. 不能是任何一个有效解析的字符
}

console.log('parser2.js');
